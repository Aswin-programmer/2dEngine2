...
bool FMODAudioSystem::LoadBank(const std::string& bankName, const std::string& bankPath)
{
    ...
    {
        LOG_WARN("FMOD: Bank '{0}' already loaded.", bankName);
        return true;
    }
    ...
    if (ErrorCheck(result) != FMOD_OK)
    {
        LOG_ERROR("FMOD: Failed to load bank '{0}' from path: '{1}'", bankName, bankPath);
        return false;
    }
    ...
    if (ErrorCheck(result) != FMOD_OK)
    {
        LOG_ERROR("FMOD: Failed to load sample data for bank '{0}'", bankName);
        bank->unload();
        return false;
    }

    m_Banks[bankName] = bank;
    LOG_INFO("FMOD: Successfully loaded bank '{0}'", bankName);
    return true;
}
...
bool FMODAudioSystem::UnloadBank(const std::string& bankName)
{
    ...
    if (it == m_Banks.end())
    {
        LOG_WARN("FMOD: Bank '{0}' not found!", bankName);
        return false;
    }
    ...
    if (ErrorCheck(result) != FMOD_OK)
    {
        LOG_ERROR("FMOD: Failed to unload bank '{0}'", bankName);
        return false;
    }

    m_Banks.erase(it);
    LOG_INFO("FMOD: Successfully unloaded bank '{0}'", bankName);
    return true;
}
...
FMOD::Studio::EventInstance* FMODAudioSystem::CreateEventInstance(const std::string& eventPath)
{
    ...
    else
    {
        ...
        {
            LOG_ERROR("FMOD: Failed to get event description for '{0}'", eventPath);
            return nullptr;
        }
    ...
    {
        LOG_ERROR("FMOD: Failed to create event instance for '{0}'", eventPath);
        return nullptr;
    }
...
FMOD::Sound* FMODAudioSystem::LoadSound(const std::string& soundPath, bool loop)
{
    ...
    {
        LOG_ERROR("FMOD: Failed to load sound '{0}'", soundPath);
        return nullptr;
    }
...
FMOD_RESULT FMODAudioSystem::ErrorCheck(FMOD_RESULT result) const
{
    if (result != FMOD_OK)
    {
        LOG_ERROR("FMOD Error: '{0}' - '{1}'", static_cast<int>(result), FMOD_ErrorString(result));
    }
